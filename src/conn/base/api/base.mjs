...
async function getConnectionBase(configOrSelector, options, initConnection) {
  ...
  if (isCalustraConnection(configOrSelector)) {
    log.warn(
      `[calustra] getConnection() called using an actual connection (${configOrSelector.connid}). Returning it.`
    )
  ...
  if (!nocache) {
    ...
    if (cachedConn) {
      ...
      if (!cachedConn.isOpen || reset) {
        log.silly(
          `[calustra] getConnection() cached connection found (${cachedConn.connid}), but, let's reinit it. Still open? ${cachedConn.isOpen}. Reset ${reset}. NoCache? ${nocache}`
        )

        await cacheConnectionUnset(cachedConn)
      } else {
        log.silly(
          `[calustra] getConnection() cached connection found (${cachedConn.connid}). Reusing it.`
        )
  ...
  if (nocache !== true) {
    ...
    log.silly(
      `[calustra] getConnection() Initing a new connection (${conn.connid}) and cached it with cache key ${cacheKey}`
    )
  } else {
    log.silly(
      `[calustra] getConnection() Initing a new connection (${conn.connid}) but not caching it.`
    )
  }
...
export { getConnectionBase, dropConnection, dropConnections, isCalustraConnection }
