...
import {
  ...
  prepare_query_select,
  prepare_query_update
} from "#conn-base/util/query/index.mjs"
...
class CalustraModelBase extends CalustraModelOptions {
  ...
  async keyList(filt, options) {
    const data = await this.read(filt, {
      fields: ["id", "name"],
  ...
  async find(id, options) {
    if (Number.isNaN(Number(id)) || id <= 0) {
  ...
  async insert(data, poptions) {
    ...
    const [params, options, allow] = await this.beforeInsert(data, poptions)

    if (!allow) return undefined

    const [query, ivalues] = prepare_query_insert(
      this.name,
      this.fields,
      params,
      /*returning*/ true
  ...
  async beforeUpdate(params, filter, options) {
    ...
    if (this.triggers.beforeUpdate !== undefined) {
      ;[params, filter, options, allow] = await this.triggers.beforeUpdate(
        this.connection,
        params,
        filter,
        options
  ...
  async update(data, filt, poptions) {
    ...
    const [params, filter, options, allow] = await this.beforeUpdate(data, filt, poptions)
    ...
    if (count === 0) {
      ...
      if (options?.log !== false) {
        this.connection.log.debug(
          `[calustra][${this.connid}] ${this.name}: Updated ${count} records`
        )
      }
    }
  ...
  async delete(filt, poptions) {
    ...
    const [filter, options, allow] = await this.beforeDelete(filt, poptions)
