...
class CalustraConnBase {
  constructor(config, options) {
    ...
    this.log.debug(
      `[calustra][${this.connid}] Opening database ${config?.database}: ${this.configDescription}${this.options?.reset === true ? " (reset)" : ""}${this.options?.cache === false ? " (nocache)" : ""}`
    )
  ...
  async _runAndLogQuery(query, values, options, run_callback, msg_callback) {
    ...
    try {
      ...
      if (options?.log !== false) {
        const level = options?.log || "debug"
        const logfunc = this.log[level].bind(this.log)
        const elapsed = parseFloat((Date.now() - started) / 1000.0).toFixed(2)
        logfunc(`[calustra][${this.connid}] ${this.formatQuery(query, values)}`)
        const msg = msg_callback(data, elapsed)
        logfunc(`[calustra][${this.connid}] ${msg}`)
      }
  ...
  async selectOne(query, values, options) {
    ...
    const omitWarning = options !== undefined ? options.omitWarning === true : false
