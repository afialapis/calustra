...
class CalustraConnPG extends CalustraConnBase {
  ...
  async execute(query, values, options) {
    const callback = () =>
      new Promise((resolve, reject) => {
        const transaction =
          options?.transaction !== undefined ? options?.transaction : this.openTransaction()
  ...
  async getTableNamesFromDb(schema = "public") {
    const query = `SELECT DISTINCT c.relname AS name
          FROM pg_class c
  ...
  async getTableDetailsFromDb(tableName, schema = "public") {
    const query = `SELECT f.attnum AS number, f.attname AS name, f.attnum,  
