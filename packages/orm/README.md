...
[CalustraOrm](http://calustra.afialapis.com/) is a small, minimalist ORM, for those out there who still feel good typing some raw SQL.
...
const CalustraOrm = require('calustra-orm');
// Using ES6 imports
import CalustraOrm from 'calustra-orm';
```

A simple way to work with `calustra-orm`: create an unique 
...
const db= CalustraOrm(config, 'warn')

export default db
```

The second parameter on the ```CalustraOrm``` call indicates the logging level.

Possible values are:

  - `undefined` or `'none'` => will disable any `calustra-orm` logging
  - `'error'`, `'warn'` or `'debug'`
...
const result= db.selectOne(qry, [1])
```
...
```javascript
import {Model, datatypes} from 'calustra-orm'
import db from './db'
```
...
There is no way to specify table relations, cause `calustra-orm` does nothing about it. Yet. 
This may change in future versions: `calustra-orm` wants to make sense out of its 'r'!
...
```javascript
const TestModel = new Model(db, 'calustra-orm_test', MODEL_DEFINITION)
