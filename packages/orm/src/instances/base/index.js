...
import { prepare_query_select, 
         ...
         prepare_queries_before_delete } from '../../util/query'
...
class CalustraModelBase extends CalustraModelConfig {
  ...
  async count(filt, options) {    
    ...
    try {
      ...
      this.connection.log.error(`${this.name} ERROR:`)
      this.connection.log.error(error.constructor.name)
  ...
  async find(id, options) {
    ...
    if (Array.isArray(data)) {
      ...
      this.connection.log.warn(`${this.name}: Id ${id} does not exist`)
    }
  ...
  async beforeInsert(params, options) {
    ...
    if (this.config.triggers.beforeInsert != undefined) {
      [params, options, allow]= await this.config.triggers.beforeInsert(this.connection, params, options)
    }
  ...
  async afterInsert(id, params, options) {
    if (this.config.triggers.afterInsert != undefined) {
      id= await this.config.triggers.afterInsert(this.connection, id, params, options)
    }
  ...
  async insert(data, poptions) {
    ...
    if (id == null) {
      ...
      if (options?.log!==false) {
        this.connection.log.debug(`Created with Id ${id}`)
      }
    }
  ...
  async beforeUpdate(params, filter, options) {
    ...
    if (this.config.triggers.beforeUpdate != undefined) {
      [params, filter, options, allow]= await this.config.triggers.beforeUpdate(this.connection, params, filter, options)
    }
  ...
  async afterUpdate(rows, params, filter, options) {
    if (this.config.triggers.afterUpdate != undefined) {
      rows= await this.config.triggers.afterUpdate(this.connection, rows, params, filter, options)
    }    
  ...
  async update(data, filt, poptions) {
    ...
    if (query == undefined) {
      this.connection.log.error(`${this.name} ERROR: Nothing to update`)
      return 0
    }
    ...
    if (count == 0) {
      ...
      this.connection.log.warn(msg)
    } else {
      if (options?.log!==false) {
        this.connection.log.debug(`Updated ${count} records`)
      }
    }
  ...
  async beforeDelete(filter, options) {
    ...
    if (this.config.triggers.beforeDelete != undefined) {
      [filter, options, allow] = await this.config.triggers.beforeDelete(this.connection, filter, options)
    }
  ...
  async afterDelete(rows, filter, options) {
    if (this.config.triggers.afterDelete != undefined) {
      rows = await this.config.triggers.afterDelete(this.connection, rows, filter, options)
    }
  ...
  async delete(filt, poptions) {
    ...
    if (options?.log!==false) {
      this.connection.log.debug(`Deleted ${count} records`)
    }    
